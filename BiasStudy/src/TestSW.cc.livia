...
#define PTL45_PTSL25_MET70_2012_FP_EB
...
int main(int argc, char* argv[]){
...
#ifdef PTL45_PTSL25_MET70_2012_FP_EB
  //provisional numbers for 2012 8TeV (not yet final xsec nor selection)
  std::map<int, double> expEvents_gluglu_map;
  expEvents_gluglu_map[110]=0.0;         
  expEvents_gluglu_map[115]=0.0;
  ...
  std::map<int, double> expEvents_wzh_map;
  expEvents_wzh_map[110]=2.89028*5.3/3.8;
  expEvents_wzh_map[115]=2.48165*5.3/3.8;
  expEvents_wzh_map[120]=2.53349*5.3/3.8;      
  expEvents_wzh_map[125]=2.32486*5.3/3.8;
  expEvents_wzh_map[130]=1.98921*5.3/3.8; //provisional
  expEvents_wzh_map[135]=1.95169*5.3/3.8; //provisional
  expEvents_wzh_map[140]=1.84537*5.3/3.8;
  expEvents_wzh_map[145]=1.70925*5.3/3.8;
  expEvents_wzh_map[150]=1.55488*5.3/3.8;
#endif
...
//   RooWorkspace *dataWS = (RooWorkspace*)inFile->Get("cms_hgg_workspace");
  ...
  RooRealVar *mass = (RooRealVar*)fitWS->var("CMS_hgg_mass");
  if(mass == NULL){
...
//   }
  ...
  RooRealVar *mu = new RooRealVar("mu","mu",0.,-20,20);
...
//   TString fitName_=fitName;
  ...
  for (int cat=0; cat<nCats; cat++){
    ...
    // ---- get data ----
    RooDataSet *temp = (RooDataSet*)fitWS->data(Form("data_mass_nocs",cat));
    if(temp == NULL){
...
//     if (genName=="pow2_2") genParamatersFcn[cat] = (RooAddPdf*)fitWS->pdf(Form("pow2_2"));
  ...
  // ---- Get mass distributions  and entries ------
    ...
    // get signal data
      ...
      TH1F *tth_hist = (TH1F *) dataWS->Get(Form("var_tth_2011_%d",mMC));
      
      if(!(gluglu_hist || vbf_hist || wzh_hist || tth_hist)){
...
//       sigDataHist->Add(tth_hist, xsec_wzh_map[mMC]);

      sigDataHist->Add(gluglu_hist);
      sigDataHist->Add(vbf_hist);
      sigDataHist->Add(wzh_hist);
      sigDataHist->Add(tth_hist);
...
//       sigData->append(*((RooDataSet*)dataWS->data(Form("var_wzh_2011_%d",mMC))));
    ...
    // get expected SM events
      ...
      sigSMEvents[mIt][cat] = sigDataHist->Integral();
...
//#endif
//#ifdef PTL45_PTSL25_MET70_2012_FP_EB
//      int nBkgExp=int(34*5.3/3.8);
//#endif
      int nBkgExp=8;
      if(data[cat] != NULL) 
	nBkgExp=data[cat]->sumEntries();
	...
	sigMCPdf[0][0]->Print();
	RooDataSet *genSim=sigMCPdf[0][0]->generate(*mass, sigGenEvents.getVal(),AutoBinned(false));
	genSim->Print();
...
//         res2->floatParsFinal().Print("s");
	...
	RooAbsReal *intBkgRange = genFcn[0]->createIntegral(*mass,NormSet(*mass),Range("sigWindow"));
	...
	RooAbsReal *int2BkgRange = genFcn[0]->createIntegral(*mass,NormSet(*mass),Range("sigWindow2"));
	...
	std::cout << normIntRange->getVal() << " , "  << normIntRange->getPropagatedError(*res) << " ," << intBkgRange->getVal()* genDatUnBin[0]->sumEntries() << std::endl;
	std::cout << normInt2Range->getVal() << " , "  << normInt2Range->getPropagatedError(*res) << " , " << int2BkgRange->getVal()*genDatUnBin[0]->sumEntries() << std::endl;
	...
	bkgTrue1fwhm_ =  intBkgRange->getVal()* genDatUnBin[0]->sumEntries();
	bkgTrue2fwhm_ =  int2BkgRange->getVal()* genDatUnBin[0]->sumEntries();
